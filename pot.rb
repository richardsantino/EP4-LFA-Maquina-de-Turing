...
class PotTuringMachine # máquina de Turing em si
  ...
  def initialize(initial_tape_value, transition_filename)
    ...
    @reading_head = 0                                         # Posiciona o leitor em zero
    ...
    @final_state = 35             # Coloca o estado final como o último estado presente no json
    @verbose = false
  end
    ...
    while @current_state != @final_state do # Enquanto não estivermos no estado final, executa esse bloco
      ...
      if @current_state == 36
        ...
        sorted_keys.each do |key|
          tape = tape + @tape[key].value.to_s
        end
        puts "Entrando na máquina de multiplicação"
        mux = MuxTuringMachine.new(tape, "mux.json", @reading_head)
        mux.verbose = @verbose 
        @tape, @reading_head = mux.run
        puts "Saindo da máquina de multiplicação"
      end
